...
export function TheoryContent() {
  return (
    ...
    <article className="prose prose-zinc max-w-none">
      ...
      <FormulaBlock caption="述語の表記">
        H(x) … Hが述語名、xが項（変数または定数）
      </FormulaBlock>
      ...
      <Callout variant="definition" label="定義">
        述語の「穴」の数を<strong>アリティ（arity）</strong>と呼びます。
        穴の数によって述語が分類されます。
      </Callout>
      ...
      <p>
        理論上はアリティに上限はありません。
        しかし実用上は一項から三項が圧倒的に多く使われます。
      </p>

      <ComparisonTable
        headers={["アリティ", "役割と例"]}
        rows={[
          ["一項述語 P(x)", "対象の性質: 「xは偶数」「xはアクティブ」"],
          ["二項述語 R(x, y)", "2者間の関係: 「xはyより大きい」「xはyの友人」"],
          ["三項述語 S(x, y, z)", "3者間の関係: 「xはyにzを送った」"],
        ]}
      />

      <KeyPoint>
        アリティは述語の「穴」の数。一項は性質、二項は関係、三項は複雑な関係を表す。
        引数の順序が意味を変えるため、順序に注意が必要。
      </KeyPoint>
      ...
      <p>
        ...
        P ∧ Q → R が成り立つかどうかは、真理値表で確認するしかありません。
        推論の<strong>構造的な必然性</strong>を捉えることができないのです。
      </p>
      ...
      <FormulaBlock caption="SQL WHERE句の例">
      ...
      <FormulaBlock caption="TypeScript型ガード = 一項述語">
        function isString(x: unknown): x is string
      </FormulaBlock>

      <p>
        この関数は「xはstring型である」という一項述語です。
        TypeScriptでは戻り値の型を <code>x is string</code> と書くことで、コンパイラに「この関数が真を返したらxはstringとして扱ってよい」と伝えます。
      ...
      <p>
        契約書の各条項は述語と項で構成されています。
        「誰が」「何に対して」「何をする」という構造を明確にしているのです。
      </p>
      ...
      <ComparisonTable
        headers={["述語であるもの", "述語でないもの"]}
        rows={[
          ["H(x): xは人間である", "「おはよう」（真偽を問えない）"],
          ["x + 3 = 7（真偽を返す）", "x + 3（数値を返す関数）"],
          ["E(x): xは偶数である", "「雨が降っている」（穴がない命題）"],
        ]}
      />

      <Callout variant="tip">
        述語は穴が空いている状態、命題は穴が埋まっている状態です。
        返り値が真偽値かどうかで述語と関数を区別しましょう。
      </Callout>

      <KeyPoint>
        述語の判別基準は2つ。(1) 穴（項を受け取る余地）があるか、(2) 穴を埋めると真偽値を返すか。
        どちらかが欠けていれば述語ではない。
      </KeyPoint>
      ...
      <KeyPoint>
        述語はSQL、TypeScript、法律文書、データベース設計のすべてに浸透している。
        述語を理解すれば、これらのツールの「なぜそう動くか」が見える。
      </KeyPoint>
      ...
      <p>
        命題論理では「すべての顧客が満足している」と「ある顧客が満足している」を区別する方法がありません。
        どちらも単に命題Pとしか書けません。
      </p>
      <p>
        述語論理では、述語 <code>S(x)</code>（xは満足している）を定義します。
      ...
      <p>
        ...
        「なぜこの条件で正しいデータが返るのか」を論理的に説明できず、パターンの丸暗記に頼るしかありません。
      ...
      <p>
        ...
        述語の概念を理解していれば、「この関数はどんな性質を判定すべきか」を明確に定義できます。
        結果として正確な型ガードを書けるようになります。
      </p>
      ...
      <p>
        ...
        「誰に対して」「何について」の責任なのか。
        述語の構造を意識すれば、契約書の曖昧な条項を見抜けるようになります。
      </p>

      <KeyPoint>
        述語を知らなければ、量化の区別・SQLの論理的理解・型ガード設計・契約書分析のすべてが困難になる。
      </KeyPoint>
      ...
      <Callout variant="warning" label="よくある誤解">
        <strong>誤解1: 述語 = 動詞</strong>
        <br /><br />
        日本語文法の「述語」は動詞や形容詞を指しますが、論理学の述語はそれよりずっと広い概念です。
        <ul>
          <li>「xは偶数である」 — 数学的性質（動詞ではない）</li>
          <li>「xはyより大きい」 — 大小関係（動詞ではない）</li>
          <li>「xはアクティブユーザーである」 — 状態（動詞ではない）</li>
        </ul>
        論理学の述語は「対象を受け取って真偽を返すもの」であり、言語学的な品詞の区分とは独立した概念です。
      </Callout>

      <Callout variant="warning" label="よくある誤解">
        <strong>誤解2: 述語は引数を1つしか取れない</strong>
        <br /><br />
        一項述語が最もシンプルなので目立ちますが、二項述語 <code>G(x, y)</code> や三項述語 <code>S(x, y, z)</code> も等しく重要です。
        とくにデータベースの外部キー（二項述語）や送金処理 <code>Transfer(送信者, 受信者, 金額)</code>（三項述語）など、実務では多項述語が頻繁に登場します。
      </Callout>

      <Callout variant="warning" label="よくある誤解">
        <strong>誤解3: 述語論理は命題論理と無関係</strong>
        <br /><br />
