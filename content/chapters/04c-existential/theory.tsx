export function TheoryContent() {
  return (
    <article className="prose prose-zinc max-w-none">
      ...
      <p>
        <code>∃x P(x)</code> は「P(x)が成り立つxが少なくとも一つ存在する」と読みます。
      ...
      <p>
        存在量化子を使った形式化で<strong>最も重要なポイント</strong>が、
        結合子の選択です。標準的な形式化パターンは以下です。
      </p>
      <p>
        <code>∃x (P(x) ∧ Q(x))</code> — 「P(x)かつQ(x)であるxが存在する」
      </p>
      <p>
      ...
      <h3>∃x(P(x)→Q(x))の罠</h3>
      <p>
        具体例で考えましょう。「ある学生はプログラマーである」を形式化したいとします。
      </p>
      <ul>
      ...
      <p>
        正しい形式化: <code>∃x (S(x) ∧ P(x))</code> — 「学生であり、かつプログラマーであるxが存在する」
      </p>
      <p>
        誤った形式化: <code>∃x (S(x) → P(x))</code> — これは何を意味するのでしょうか？
      </p>
      <p>
        含意（→）の真理値表を思い出してください。<code>S(x) → P(x)</code> は、
        S(x)が偽のとき<strong>自動的に真</strong>になります。
        つまり、学生でない対象（例えば石ころ、猫、東京タワーなど）が
        一つでも世界に存在すれば、その対象についてS(x)→P(x)は真になります。
      </p>
      <p>
        したがって、<code>∃x (S(x) → P(x))</code> は
      ...
      <ul>
        <li>
          <strong>全称量化子 ∀</strong>: <code>∀x (P(x) → Q(x))</code>
          — 「Pであるものはすべて、Qでもある」
        </li>
        <li>
          <strong>存在量化子 ∃</strong>: <code>∃x (P(x) ∧ Q(x))</code>
          — 「PでありかつQであるものが存在する」
        </li>
      </ul>
      ...
      <ul>
        <li><code>∃x Bug(x)</code> — 「バグが存在する」</li>
        <li><code>∃x (Bug(x) ∧ Critical(x))</code> — 「致命的なバグが存在する」</li>
        <li><code>∃x (Bug(x) ∧ InModule(x, auth))</code> — 「認証モジュールにバグが存在する」</li>
      </ul>
      <p>
      ...
      <h3>数学: ある素数は偶数である</h3>
      <p>
        <code>∃x (Prime(x) ∧ Even(x))</code> — 「素数であり、かつ偶数であるxが存在する」
      </p>
      <p>
        これは真です。x = 2がその証拠です。2は素数であり、偶数でもあります。
        存在証明では、条件を満たす具体例を一つ提示すれば十分です。
        なお、∃は「少なくとも一つ」を意味するだけであり、
        この場合は偶数の素数が2だけですが、∃の主張としてはそれで十分です。
      </p>
      ...
      <p>
        <code>users.some(u =&gt; u.isAdmin)</code>
        は「管理者であるユーザーが少なくとも一人存在するか？」を判定します。
        これは <code>∃x (User(x) ∧ Admin(x))</code> と同じ構造です。
      </p>
      <p>
        同様に、<code>Array.find()</code> は存在量化子の「証拠」を返す関数です。
        <code>users.find(u =&gt; u.isAdmin)</code> は
        「Admin(x)を満たす具体的なxを一つ見つけて返す」操作です。
      </p>
      ...
      <p>
        <code>SELECT * FROM departments d WHERE EXISTS (SELECT 1 FROM employees e WHERE e.dept_id = d.id AND e.salary &gt; 100000)</code>
      </p>
      <p>
      ...
      <p>
        <code>∃x (Student(x) → Fly(x))</code> — 「学生ならば空を飛べる、そういうxが存在する」
      </p>
      <p>
        一見すると「空を飛べる学生がいる」という意味に見えます。しかし実際には、
        この文は<strong>ほぼ確実に真</strong>です。なぜなら：
      </p>
      <ul>
      ...
      <p>
        学生が空を飛べるかどうかとは無関係に、学生でないものが一つでも存在するだけで真になります。
      ...
      <p>
        ...
        逆に「バグがない」と主張するには、すべてのケースをテストする必要があります
        （∀x ¬Bug(x)）。これが網羅的テストの難しさの本質です。
      </p>
      ...
      <p>
        ...
        EXISTS句の意味が分からなければ、サブクエリの設計が曖昧になり、
      ...
      <p>
        ...
        と書きます。これは「P(x)を満たすxが存在し、かつP(y)を満たすyはすべてそのxと同一である」
        という意味で、<strong>唯一存在量化子（∃!）</strong>と呼ばれます。
      </p>

      <h3>誤解2: ∃x(P(x)→Q(x))が正しい形式化だ</h3>
      <p>
        前述の通り、これは空虚な真の罠に陥ります。
      ...
      <p>
        ...
        特に∃x(P(x)→Q(x))は、世界にP(x)が偽のものが一つでもあれば真になるため、
      ...
      <ul>
        <li>
          <strong>¬∃xP(x)</strong> = 「P(x)を満たすxは一つも存在しない」
          = <code>∀x ¬P(x)</code>（すべてのxについてP(x)は偽）
        </li>
        <li>
          <strong>∃x¬P(x)</strong> = 「P(x)を満たさないxが少なくとも一つ存在する」
          （P(x)が真のxも存在するかもしれない）
        </li>
      </ul>
      <p>
        例で考えましょう。P(x) = 「xは合格した」とすると：
      </p>
      <ul>
        <li>¬∃xP(x) = 「合格者は一人もいない」（全員不合格）</li>
        <li>∃x¬P(x) = 「不合格者が少なくとも一人いる」（合格者もいるかもしれない）</li>
      </ul>
      <p>
